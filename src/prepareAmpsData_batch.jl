...
for fileName in readdir(filePath)
    if !contains(fileName, ".dat")
        continue
    end
    fileNameExtension = split(fileName, ".")[end]
    # subtract extension and last dot from filename
    fileNameBase = basename(fileName)[1:end-(length(fileNameExtension)+1)]

    nCells, nCellsPerBlock, nBlocks, nodeCoordinates, cubeIndices, numberDensity, minSize, maxSize, varNames = load_AMPS_data(joinpath(filePath,fileName))
